 ...
 * Date: 04/05/2011
 * Compile as follows: gcc -o signalling_using_condition_variables_v2 signalling_using_condition_variables_v2.c -std=c99 -lpthread
 */
...
void *waiting_thread(void *args)
{
		  ...
		  if(data.value == 0){
					 ...
					 pthread_cond_wait(&data.condition, &data.mutex); // Release mutex and wait here for condition to be signalled
		  }
...
void *signalling_thread(void *args)
{
		  int status;
