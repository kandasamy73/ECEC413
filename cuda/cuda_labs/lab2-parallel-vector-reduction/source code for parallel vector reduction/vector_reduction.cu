...
// We are fixing the problem size to five million elements.
...
runTest( int argc, char** argv) 
{
   ...
   // Randomly generate input data and compare against the host's result.
   // Initialize the input data on the host to be integer values between 0 and 5
   srand(time(NULL));  
	for(unsigned int i = 0; i < num_elements; ++i){
		h_data[i] = floorf(5*(rand()/(float)RAND_MAX));
	}
