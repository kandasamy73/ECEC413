 ...
 * Date: 04/15/2011
 * Compile as follows: gcc -o trap_v3 trap_v3.c -fopenmp -std=c99
 */
...
#pragma omp parallel num_threads(thread_count) reduction(+: approximate_area)   /* OpenMP block */

    {
        approximate_area += trap_per_thread(a, b, n);
    }
...
double 
f(double x)                 /* Function to be integrated. */
{
    return((x*x*x + x*x + 1)/(x - 1));
}

double 
trap_per_thread(double a, double b, int n)
{
    int my_id = omp_get_thread_num();
    int thread_count = omp_get_num_threads();

    double h = (b - a)/(float)n;                /* Length of the subinterval. */
    int chunk_size = n/thread_count;            /* We assume that the number of subintervals is evenly divisible by thread_count. */
